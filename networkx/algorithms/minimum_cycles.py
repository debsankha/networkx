...
========================
We compute the minimum cycle basis for an undirected graph 
Using de Pina's algorithm.
"""
...
__all__ = ['minimum_cycle_basis']

__author__ = "\n".join(['Debsankha Manik <dmanik@nld.ds.mpg.de>'])
...
def minimum_cycle_basis(G, weight=None):
    ...
    for comp in nx.connected_component_subgraphs(G):
        ...
        for k in range(N):
            ...
            # orthogonal to the newly found cycle
            set_orth = _update_orth(set_orth, comp, new_cycle, k)
...
def _update_orth(set_orth, graph, new_cycle, k):
    """
    Updates the set of vectors orthogonal to the first k basis cycles,
    as per [p. 336, 1]
    """
    for orth in set_orth[k + 1:]:
        if len(set.intersection(orth, new_cycle)) % 2:
            orth = orth.symmetric_difference(set_orth[k])
        else:
            pass
