...
from networkx.algorithms import minimum_cycles as mc
...
class TestMinimumCycles:
    ...
    def test_unweighted_diamond(self):
        mcb = mc.minimum_cycle_basis(self.diamond_graph)
        self.basis_equal(mcb, [[1, 2, 4], [2, 3, 4]])

    def test_weighted_diamond(self):
        mcb = mc.minimum_cycle_basis(self.diamond_graph, weight='weight')
    ...
    def test_dimentionality(self):
        ...
        for _ in range(ntrial):
            ...
            dim_mcb = len(mc.minimum_cycle_basis(rg))
            assert(dim_mcb == nedges - nnodes + ncomp)

    def test_complete_graph(self):
        cg = nx.complete_graph(5)
        mcb = mc.minimum_cycle_basis(cg)
        all([len(cycle) == 3 for cycle in mcb])

    def test_tree_graph(self):
        tg = nx.balanced_tree(3, 3)
        assert_false(mc.minimum_cycle_basis(tg))
